/*
Problem: Objects, as generic blobs of values, can be used to build all sorts of data structures. A common 
data structure is the list (not to be confused with array). A list is a nested set of objects, with the 
...
The resulting objects form a chain, like this: *A linked list*
...
argument. Also write a listToArray function that produces an array from a list. Then add a helper 
...
const listToArray = function(list) {
    ...
    let next = {...list}; 
    while (next) {
        convertedArray.push(next.value);
        next = next.rest;
    }
...
const recursiveNth = function(list, index) {
    if (index === 0) {
        return list.value;
    } else {
...
// → 20
console.log(recursiveNth(arrayToList([10, 20, 30, 15, 25, 35]), 5));
// → 35